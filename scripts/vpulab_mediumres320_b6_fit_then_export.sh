...
THRESHOLD="${THRESHOLD:-0.890}"

cd "${ROOT}"
mkdir -p submissions features models/vpulab_queue_logs
LOG="models/vpulab_queue_logs/mediumres320_b6_fit_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "${LOG}") 2>&1

echo "[$(date)] medium-res 320x288 b6 fit started"
echo "will export only if train Acc reaches >= ${THRESHOLD}"
nvidia-smi || true
...
mapfile -t CANDIDATES < <(
  grep -E "Epoch\\[[0-9]+\\] Iteration\\[[0-9]+/[0-9]+\\].*Acc:" "models/${LOG_NAME}/train_log.txt" \
    | sed -E 's/.*Epoch\\[([0-9]+)\\].*Acc: ([0-9.]+).*/\\1 \\2/' \
    | awk -v threshold="${THRESHOLD}" '$2 >= threshold {print $1, $2}' \
    | awk '!seen[$1]++'
)

if [ "${#CANDIDATES[@]}" -eq 0 ]; then
  echo "[$(date)] no epoch reached Acc >= ${THRESHOLD}; not exporting"
  exit 0
fi

read -r EXPORT_EPOCH EXPORT_ACC <<<"${CANDIDATES[0]}"
echo "[$(date)] first exportable epoch: ${EXPORT_EPOCH} with train Acc ${EXPORT_ACC}"

"${PY}" tools/export_submission_fast.py \
  --config_file config/UrbanElementsReID_test.yml \
  --track "submissions/${LOG_NAME}_e${EXPORT_EPOCH}_acc${EXPORT_ACC}_fast.txt" \
  ...
  INPUT.ASPECT_PAD.ENABLED True \
  DATALOADER.NUM_WORKERS 0 \
  TEST.WEIGHT "models/${LOG_NAME}/part_attention_vit_${EXPORT_EPOCH}.pth" \
  TEST.IMS_PER_BATCH 32 \
  TEST.QUERY_EXPANSION False \
  TEST.CLASS_POSTPROCESS off \
  TEST.CLASS_MISMATCH_PENALTY 0.0 \
  TEST.FEAT_Q_PATH "features/${LOG_NAME}_e${EXPORT_EPOCH}_qf.npy" \
  TEST.FEAT_G_PATH "features/${LOG_NAME}_e${EXPORT_EPOCH}_gf.npy" \
  LOG_NAME "submissions/${LOG_NAME}_e${EXPORT_EPOCH}_acc${EXPORT_ACC}_fast"

echo "[$(date)] medium-res 320x288 b6 fit/export finished"
