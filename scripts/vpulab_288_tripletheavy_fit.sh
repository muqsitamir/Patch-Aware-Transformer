...
LOG_NAME="vitl_aspectpad_288x256_e6_tripletheavy_margin_e4"
THRESHOLD="${THRESHOLD:-0.890}"

cd "${ROOT}"
mkdir -p submissions features models/vpulab_queue_logs
LOG="models/vpulab_queue_logs/288_tripletheavy_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "${LOG}") 2>&1

echo "[$(date)] 288x256 triplet-heavy run started"
echo "export threshold=${THRESHOLD}"
nvidia-smi || true
...
echo "[$(date)] 288x256 triplet-heavy run finished"
